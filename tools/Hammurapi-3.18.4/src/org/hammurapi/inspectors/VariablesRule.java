 ...
 */
public class VariablesRule extends InspectorBase  {
	
	public void visit(VariableDefinition element) {
        AST node=element.getAst();
        AST prevNode=(AST) node.getPrevSibling();
        if (
            prevNode!=null && 
			!(element.getParent() instanceof ForStatement) && 
            "VARIABLE_DEF".equals(prevNode.getTypeName()) &&
            node.getLine()==prevNode.getLine() && 
            node.getColumn()==prevNode.getColumn()            
        ){
